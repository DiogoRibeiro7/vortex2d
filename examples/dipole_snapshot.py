...
def main() -> None:
    ...
    tracers = PassiveTracers2D(positions=np.stack([np.linspace(-0.25, 0.25, 41), np.zeros(41)], axis=1))
    for _ in range(800):
        dt = sim.suggest_dt(cfl=0.25, floor=5e-4)
        sim.step(dt)
        tracers.step(sim, dt)

    plot_snapshot(sim, tracers=tracers, domain=(-0.6, 0.6, -0.45, 0.45), nx=96, ny=72, quiver_subsample=8)
